...
From the table above, account "1049333510505291778" appear in most centrality. That account has the most degree in the network (high degree) but also surrounded by important person (high eigenvector). TeamAlgoritma is an exception, that's our ego querry so it isn't wise to make it as key player in its own ego network. We can conclude that user "1049333510505291778" is the key player of TeamAlgoritma Twitter ego network, but i need further clarification for this. Eigenvector centrality is highly corelated with degree centrality. Degree centality barely tell us nothing except how many connection they have. i want look at user that appear in the top of most centrality. in this case, user "1025350578505838592" and "483847503" will be our best candidate for key player. Those user appear as top 4 in all centrality. This is where subjective viewpoint comes into play. for me, i'll choose user "1025350578505838592" and "483847503" as TeamAlgoritma ego network Key player

Let's see who he/she is:
```{r}
key_player_ego <- lookup_users(c("1025350578505838592","483847503"))
key_player_ego
```
...
kp_activity <- data.frame(
  network_act_df %>% arrange(-degree) %>% select(name) %>% slice(1:5),
  network_act_df %>% arrange(-between) %>% select(name) %>% slice(1:5),
  network_act_df %>% arrange(-closeness) %>% select(name) %>% slice(1:5),
  network_act_df %>% arrange(-eigen) %>% select(name) %>% slice(1:5)
) %>% setNames(c("degree","betweenness","closeness","eigen"))

kp_activity
```

From the table above, "Timothy_Hughes" grab our attention the most. he/she appear in most centrality. Thus we can conclude he/she is the key player in #NewNormal Twitter actiity network. Let's see his tweets
```{r}
newnorm %>% filter(screen_name == "Timothy_Hughes") %>% arrange(-retweet_count)
```
