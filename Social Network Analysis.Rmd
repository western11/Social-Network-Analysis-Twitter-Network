...
**Note:** In 28 March 2019, Kak Eca already deliver a cool internal training about Twitter Interactions using `Twinetverse` <will input the link later>. It has similar idea with this study, we both explain about building network from Twitter data. The difference in this study is we're not only make visualization, but also calculate metrics to achieve the goals for every case (spoiler: we'll do 2 SNA cases). We'll also more focus to how to implement SNA and what it is. This study is theory-driven studies.
...
In this study, we'll crawl Twitter data without using access token as crendentials. but if things are going bad, i provide some access token we can use to crawl the data. **Note:** Due to rate limit, and most of us will use it at the same time, please use the token wisely.
...
## Graph Theory
In mathematics, graph theory is the study of graphs, which are mathematical structures used to model pairwise relations between objects. A graph in this context is made up of **vertices** (also called nodes or points) which are connected by **edges** (also called links or lines). In general, A connection between nodes are seperated by 2 types: [Directed and Undirected](https://www.mathworks.com/help/matlab/math/directed-and-undirected-graphs.html). 

- Directed is a relationship between nodes that the edges has direction (The edges have orientations). You will recognize it as edges that has arrow in it. Directed network also separated in 2 type based on its direction, namely: in-degree and out-degree. In-degree represents the number of edges incoming to a vertex/node. In below directed graph, In-degree of A is 1 and degree of D is 2. Out-degree represents the number of edges outgoing from a vertex. In below directed graph, out-degree of A is 1 and out-degree of C is 3.
```{r, echo=FALSE}
set.seed(123)
...
- Undirected indicates two-way relationship, the edges are unidirectional, with no direction associated with them. Hence, the graph can be traversed in either direction. The absence of an arrow tells us that the graph is undirected.
```{r,echo=FALSE}
...
Same like before, we build loop function to gather the following. in `rtweet` package, following is also called as `friend`.
```{r}
...
It isn't done yet. this is an ego network for TeamAlgoritma account, we want that account to appear in our screen. since TeamAlgoritma is barely follow back is followers, it's not a surprise if we can't found it in mutual dataframe 
```{r}
...
### Degree Centrality   
The easiest centrality among them all. Its just **how many ties that a node has**. The calculation for directed and undirected are kinda different but it have a same idea: how many nodes is connected to a node. 
...
From the table above, account "1049333510505291776" appear in most centrality. That account has the most degree in the network (high degree) but also surrounded by important person (high eigenvector). TeamAlgoritma is an exception, that's our ego querry so it isn't wise to make it as key player in its own ego network. We can conclude that user "1049333510505291776" is the key player of TeamAlgoritma Twitter ego network.


**Note:** after lots of troubleshooting, it seems that user id "1049333510505291776" has been changed to "1049333510505291778". I use old data when i do this analysis (15 may 2020) and twitter user id is changed overtime. 
...
## Visualize Network
Let's try visualize the network. I'll scale the nodes by degree centrality, and color it by community. since our network is too large (approximately 14k nodes and 15k edges), i'll filter by only showing community 1 - 3. 
...
This obviously doesn’t tell much of a story (we need further inspection in the data, matching it to the visualization), but it shows that the “random walk” community detection algorithm is picking up on the same structure as "stress" layout algorithm. TeamAlgoritma as our ego appear in the middle, act as a bridge who connects all cluster. we only show user label who has high betweenness centrality value. a mushroom-shape nodes behind them are their mutual friends who doesn't follow TeamAlgoritma account. That user is our potential reader if their 'bridge' retweeting or mentionin something about TeamAlgoritma account. user in same community or who close to each other maybe know each other in real life. they create their own community. both key player are in a same community (community #1 i presume), that is TeamAlgoritma's most important community because they have the most potential to spread information fast and widely. 

# Case 2: Activity Network
Activity network analyze how information is spread around the network. From this analysis, we can found how something goes viral and how people interact with it. Because this network contain all form of Twitter communication (even a single retweet), the visualization will most likely become a hairball. You will need a lot of data featuring or filtering for visualization purpose, but in the end it will be up to your preferences.   
...
3. Specify which column is communicate with which, then create edges df   
4. Create nodes df and the network   
5. Build communities, Calculate SNA metrics, and identify which user is important   
6. Visualize the network    

## Gather tweets data
You can always choose another keywords or hashtag. in this example, i'll use #NewNormal as the querry.
```{r, eval=FALSE}
...
## Identify prominent person in the network
Key player in ego network and activity network have different intepretation. In this case, the key player will be considered as person who make this specific keyword viral. We also can identify which person whose opinions are widely agreed upon by others. 
...
kp_activity
```
We got interesting result here. every top centrality have different account. It means every top user is great in their own 'job' on the network. we can tell that there's no super user in #NewNormal network. From the table above, "adrianoqalbi" grab my attention the most. he appear in 2 centrality (if we remove NA in eigenvector centrality). Thus we can conclude he is the key player in #NewNormal Twitter actiity network. Let's see his tweets
```{r}
newnorm %>% filter(screen_name == "adrianoqalbi") %>% arrange(-retweet_count)
```

## Visualize network
let's try to visualize the network. the network consist 23k+ nodes and  17k+ degree. we need to filter it otherwise it'll be just hairball of nodes and edges. here i just plot top 3 community and filter the label by nodes with degree centrality >= 3 or betweenness centrality >= 100. The edges color are seperated by interaction type.
...
```
Now we can identify which person is belong to which community based on highest degree. The plot is arranged by community and degree, so user in same community is placed near to each other. community #1 (highest community by frequency) are the only community with 2 interaction types. lots of them are retweeting (or quoting) "adrianoqalbi" tweet, and the other are people who mention "ari_maj" in ther tweets. User in community #2 and #3 are just retweeting each other, or maybe some of them make a quote tweet that also goes viral. There's a news account like (detikcom) in community #3, it'll be our evidence that those communities are driven by retweet. I can conclude that top 3 community (by highest frequency) in Twitter activity network is well separated by its tweet types (interactoin types). maybe there's another interesting insight in another community since it creates fewer account by each community.



# Another additional case: Competition network 
