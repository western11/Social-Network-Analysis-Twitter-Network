---
title: "Social Network Analysis on Twitter"
author: "Joe Cristian"
github: "https://github.com/western11/Social-Network-Analysis-Twitter-Network"
date: '2020-05-18'
slug: social-network-analysis-on-twitter
categories:
  - R
tags: 
  ...
  - ggraph
  - Data Visualization
description: 'Brief introduction of Social Network Analysis (SNA) and its implementation on Twitter Network'
featured: ''
featuredalt: ''
featuredpath: ''
linktitle: ''
...
<center> ![](sna-on-twitter/mindmap.png) </center>
...
```
```{r,echo=FALSE}
mytoken_1 <- readRDS("sna-on-twitterdata/token_1.rds")
```
...
<center> ![Picture 1: Ego network ](sna-on-twitter/ego_network.png){width="60%"} </center>
...
active_fol <- read.csv("sna-on-twitterdata/active_fol.csv")
```
...
```{r}
algo_follower_df <- read.csv("sna-on-twitterdata/follower_algo.csv") 
```
...
```{r}
active_fol %>% arrange(-friends_count) %>% slice(1:10)
```
...
```{r}
algo_friend_df <- read.csv("sna-on-twitterdata/following_algo.csv")
```
...
```{r}
product_df <- read.csv("sna-on-twitterdata/product_exnet.csv",stringsAsFactors = F)
...
plot_ego1 <- readRDS("sna-on-twitterdata/plotego.rds")
plot_ego1
```
...
```{r}
newnorm <- read.csv("sna-on-twitterdata/newnormal.csv", stringsAsFactors = F)
```
